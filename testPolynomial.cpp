...
UnitTest (PolynomialDefaultConstructor) {
	...
	assertThat (bad.begin(), isEqualTo(bad.end()));
	assertTrue (bad.sanityCheck());
}
...
UnitTest (PolynomialLinearConstructor) {
	...
	std::array<Term, 2> expected {Term(-1, 0), Term(1, 1)};
	assertThat (contentsOf(xm1), matches(contentsOf(expected)));

	assertTrue (xm1.sanityCheck());

}

UnitTest (PolynomialTermConstructorNoZeros) {
	Term term(2, 3);   // 2 x^3

	Polynomial p3({term, Term(1, 0)}); // 2x^3 + 1
	assertThat (p3.getDegree(), is(3));
	assertThat(p3.getCoeff(0), is(1));
	assertThat(p3.getCoeff(1), is(0));
	assertThat(p3.getCoeff(2), is(0));
	assertThat(p3.getCoeff(3), is(2));

	Polynomial p5({Term(1, 0), term}); // 1 + 2x^3
	assertThat(p3, isEqualTo(p5));

	ostringstream out2;
	out2 << p3;
	assertThat(out2.str(), is("2x^3 + 1"));

	std::array<Term,2> expected {Term(1,0), Term(2, 3)};
	assertThat(contentsOf(p3), matches(contentsOf(expected)));

	assertTrue (p3.sanityCheck());

}


UnitTest (PolynomialTermConstructorInnerZeros) {
	Term term(2, 3);   // 2 x^3
	Polynomial p({term});
	...
	int arr2[] {1, 0, 0, 2};	
	Polynomial p4(4, arr2);
	assertThat(p3, isEqualTo(p4));
	assertThat(p3, isNotEqualTo(p));


}


UnitTest (PolynomialTermConstructorUnNormal) {
	Term term(2, 3);   // 2 x^3

	Polynomial p3({term, Term(1, 0)}); // 2x^3 + 1

	Polynomial p6 ({Term(5, 1), Term(1, 0), Term(-5, 1), Term(2, 3)}); // 5x + 1 - 5x + 2x^3
	assertThat(p6, isEqualTo(p3));
	assertTrue(p6.sanityCheck());

	Polynomial p7({Term(5, 1)});
	Polynomial p8({Term(0, 2), Term(5, 1), Term(0, 3)});
	assertThat (p8.getDegree(), is(1));
	assertThat(p8.getCoeff(0), is(0));
	assertThat(p8.getCoeff(1), is(5));
	assertThat(p8.getCoeff(2), is(0));
	assertThat(p8.getCoeff(3), is(0));

	assertThat(p7, isEqualTo(p8));
	assertThat(p8, isNotEqualTo(p3));
	
	Polynomial p9({Term(-1, 3), Term(0, 2), Term(5, 1), Term(1, 3)});
	assertThat (p9.getDegree(), is(1));
	assertThat(p9.getCoeff(0), is(0));
	assertThat(p9.getCoeff(1), is(5));
	assertThat(p9.getCoeff(2), is(0));
	assertThat(p9.getCoeff(3), is(0));

	assertThat(p7, isEqualTo(p9));
	assertThat(p8, isEqualTo(p9));
	assertThat(p9, isNotEqualTo(p3));
...
UnitTest (PolynomialArrayConstructorBasic) {
	...
	{
		assertThat(it->coefficient, isEqualTo(arr[it->power]));
	}
	const Polynomial p2(p);
	for (auto it = p2.begin(); it != p2.end(); ++it)
	{
		assertThat(it->coefficient, isEqualTo(arr[it->power]));
	}

	assertTrue (p.sanityCheck());
}
